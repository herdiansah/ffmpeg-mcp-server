/**
 * FFmpeg MCP Server
 * Developed by Herdiansah - FlashID
 */
...
server.tool(
    ...
    "Convert an MP4 video file to a sequence of images (PNG or JPG) using FFmpeg.",
    {
        input_path: z.string().describe("Absolute path to the input .mp4 file."),
        output_dir: z.string().describe("Directory to save the output images."),
        format: z.enum(["png", "jpg"]).default("png").describe("Output format (png or jpg). Default is png."),
        frame_rate: z.number().optional().describe("Frames per second to extract. If omitted, extracts all frames."),
    },
    async ({ input_path, output_dir, format, frame_rate }) => {
        ...
        if (format === "png") {
            ...
            args.push(path.join(output_dir, "output_%04d.png"));
        } else {
            // JPG, smaller size, no transparency
            args.push(path.join(output_dir, "output_%04d.jpg"));
        }
